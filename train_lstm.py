...
def train_univariate_lstm(df, seq_length=3):
    ...
    model.save('models/univariate_lstm.keras')
...
def train_multivariate_lstm(df, seq_length=3):
    ...
    model.save('models/multivariate_lstm.keras')
    return model, history
...
if __name__ == "__main__":
    ...
    # Normally we would create sequences *per player* to avoid predicting player B from player A's history.
    # We implement that refinement here
    print("Preparing sequences per player...")
    seq_length = 3
    X_uni, y_uni = [], []
    X_multi, y_multi = [], []
    ...
    for player_id, group in df.groupby('player_id'):
        ...
        # Univariate
        data_u = group[['market_value_scaled']].values
        xu, yu = create_sequences(data_u, data_u, seq_length)
        X_uni.extend(xu)
        y_uni.extend(yu)
        
        # Multivariate
        data_xm = group[multi_cols].values
        xm, ym = create_sequences(data_xm, data_u, seq_length)
        X_multi.extend(xm)
        y_multi.extend(ym)
    ...
    print(f"Total sequences created: {len(X_uni)}")
    
    # Univariate Split
    Xu_train, Xu_test, yu_train, yu_test = train_test_split(X_uni, y_uni, test_size=0.2, shuffle=False)
    ...
    hist_u = model_u.fit(Xu_train, yu_train, epochs=15, batch_size=64, validation_data=(Xu_test, yu_test), verbose=1)
    model_u.save('models/univariate_lstm.keras')
    
    # Multivariate Split
    Xm_train, Xm_test, ym_train, ym_test = train_test_split(X_multi, y_multi, test_size=0.2, shuffle=False)
    
    print("\n--- Training Multivariate ---")
    model_m = Sequential([
    ...
    hist_m = model_m.fit(Xm_train, ym_train, epochs=15, batch_size=64, validation_data=(Xm_test, ym_test), verbose=1)
    model_m.save('models/multivariate_lstm.keras')
    
    # Saving histories for plotting later
    np.save('models/hist_uni.npy', hist_u.history)
    np.save('models/hist_multi.npy', hist_m.history)
    ...
    enc_dec_model.save('models/encoder_decoder_lstm.keras')
    np.save('models/hist_enc_dec.npy', hist_ed.history)
